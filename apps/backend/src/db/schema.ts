...
export const paymentRequests = sqliteTable(
  ...
  table => ({
    ...
    currentApproverIdx: index('idx_payment_requests_current_approver').on(
      table.currentApprover
    ),
  })
);
...
export const invoicePaymentRequests = sqliteTable(
  'invoice_payment_requests',
  {
    ...
    linkedAt: integer('linked_at', { mode: 'timestamp' }).default(
      sql`(unixepoch())`
    ),
  },
  table => ({
    invoiceIdx: index('idx_invoice_payment_requests_invoice').on(
      table.invoiceId
    ),
    paymentRequestIdx: index('idx_invoice_payment_requests_pr').on(
      table.paymentRequestId
    ),
    uniqueLink: index('idx_invoice_payment_requests_unique').on(
      table.invoiceId,
      table.paymentRequestId
    ),
  })
...
export const approvalHistory = sqliteTable(
  ...
  table => ({
    paymentRequestIdx: index('idx_approval_history_payment_request_id').on(
      table.paymentRequestId
    ),
...
export const businessProcessLogs = sqliteTable(
  ...
  table => ({
    processIndexIdx: index('idx_business_process_logs_process_index').on(
      table.processIndex
    ),
    entityIdx: index('idx_business_process_logs_entity').on(
      table.entityType,
      table.entityId
    ),
    statusIdx: index('idx_business_process_logs_status').on(table.status),
    errorCodeIdx: index('idx_business_process_logs_error_code').on(
      table.errorCode
    ),
  })
);
...
export type NewInvoicePaymentRequest =
  typeof invoicePaymentRequests.$inferInsert;
